...
func EnsureLatestManagementHTML(ctx context.Context, staticDir string, proxyURL string, panelRepository string) bool {
	...
	if HasEmbeddedManagementHTML() {
		if err := reconcileEmbeddedManagementHTML(staticDir, localPath); err != nil {
			log.WithError(err).Warn("failed to reconcile embedded management asset")
		}
...
// reconcileEmbeddedManagementHTML writes the compiled-in control panel to disk
// when the local copy is missing or differs from the embedded bytes. It performs
// no network I/O. Safe to call repeatedly (idempotent once the file matches).
func reconcileEmbeddedManagementHTML(staticDir, localPath string) error {
	data := EmbeddedManagementHTML()
	if len(data) == 0 {
		return fmt.Errorf("embedded management asset is empty")
	}
	...
	if localHash, err := fileSHA256(localPath); err == nil && strings.EqualFold(localHash, embeddedHash) {
