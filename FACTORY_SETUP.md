# Using Factory AI with VibeRouter

A simplified guide for using Factory CLI (Droid) with your personal Claude and ChatGPT subscriptions through VibeRouter.

## What is This?

This guide shows you how to use [Factory CLI](https://app.factory.ai/r/FM8BJHFQ) with your personal Claude Code Pro/Max and ChatGPT Plus/Pro subscriptions instead of paying for separate API access. VibeRouter acts as a bridge that handles authentication and routing automatically.

**How it works:**

```
Factory CLI  →  VibeRouter  →  [OAuth Authentication]  →  Claude / ChatGPT APIs
```

VibeRouter manages OAuth tokens, auto-refreshes them, routes requests, and handles API format conversion — all automatically in the background.
...
## Step 1: Install VibeRouter

1. **Download [VibeRouter](https://github.com/Geekyshubham/vibeproxy-ultra/releases)** from the releases page or build from source
2. **Install**: Drag `VibeRouter.app` to your `/Applications` folder
3. **Launch**: Open VibeRouter from Applications
   - If macOS blocks it: Right-click → Open, then click "Open" in the dialog

## Step 2: Connect Your Accounts

Once VibeRouter is running:

1. Click the **VibeRouter menu bar icon**
2. Select **"Open Settings"**
3. Click **"Connect"** next to Claude Code
   - Your browser will open for authentication
   - Complete the login process
   - VibeRouter will automatically detect when you're authenticated
4. Click **"Connect"** next to Codex
   - Follow the same browser authentication process
   - Wait for VibeRouter to confirm the connection
5. **(Optional)** Click **"Connect"** next to Antigravity
   ...
   - VibeRouter will automatically save your credentials
...
8. **(Optional)** Click **"Add Account"** next to Z.AI GLM
   ...
   - VibeRouter will securely store your API key
...
3. **Start coding!** Factory will now route all requests through VibeRouter, which handles authentication automatically.
...
If you have a GitHub Copilot subscription, VibeRouter can route requests through GitHub's API, giving you access to many of the same models via a separate quota.
...
> [!NOTE]
> Copilot model names use **dot-notation** (`claude-opus-4.6`) while Claude Code subscription models use dash-notation (`claude-opus-4-7`). Both work through VibeRouter but route through different auth backends. Use `provider: "openai"` and `base_url` with `/v1` for Copilot models.
...
> - **Available in**: VibeRouter v1.0.9+ with CLIProxyAPI 6.5.1+
> 
> Connect to Antigravity in VibeRouter Settings → Click "Connect" next to Antigravity → Sign in with your Google account. After connecting, restart VibeRouter to activate Gemini 3.1 / 3 Pro access.
...
> [!NOTE]
> Z.AI GLM models require an API key instead of OAuth authentication. Get your API key at [z.ai/manage-apikey/apikey-list](https://z.ai/manage-apikey/apikey-list) and add it in VibeRouter Settings → Z.AI GLM → Add Account.
...
No manual CLIProxyAPI update is required—VibeRouter automatically keeps CLIProxyAPI up to date via our new auto-update workflow, so you can use new models immediately.

## Troubleshooting

### VibeRouter Menu Bar Status
...
| Problem | Solution |
|---------|----------|
| Can't connect to Claude/Codex/Gemini | Re-click "Connect" in VibeRouter settings |
| Factory shows 404 errors | Make sure VibeRouter server is running (check menu bar) |
| Authentication expired | Disconnect and reconnect the service in VibeRouter |
| Port 8317 already in use | Quit any other instances of VibeRouter or CLIProxyAPI |
| Gemini returns 401 errors | Verify your Google Cloud project has Gemini API enabled |

### Verification Checklist

1. ✅ VibeRouter is running (menu bar icon shows green)
...
> [!NOTE]
> The `-thinking-NUMBER` model naming convention is a **VibeRouter-specific implementation**, not an official Claude model name from Anthropic. VibeRouter intercepts these custom model names and translates them into proper API calls with the `thinking` parameter.

VibeRouter automatically adds extended thinking support for Claude models! Simply append a thinking suffix to any Claude model name:
...
**How It Works**:
1. VibeRouter's thinking proxy intercepts requests on port 8317
...
If the suffix is not a valid integer (e.g., `-thinking-blabla`), VibeRouter strips the suffix and uses the vanilla model without thinking.
...
When you use extended thinking (`-thinking-*` suffix), VibeRouter automatically enables **interleaved thinking** by adding the `anthropic-beta: interleaved-thinking-2025-05-14` header to your requests.
...
You don't need to configure anything - when you use any `-thinking-*` model variant, VibeRouter automatically:
...
- **Launch at Login**: Enable in VibeRouter settings to auto-start the server
- **Auth Folder**: Click "Open Folder" in settings to view authentication tokens
- **Server Control**: VibeRouter automatically stops the server and releases port 8317 when you quit

## Security

- All authentication tokens are stored locally in `~/.cli-proxy-api/`
- Token files are secured with proper permissions (0600)
- VibeRouter only binds to localhost (127.0.0.1)
...
> [!WARNING]
> <br>**By using this VibeRouter, you acknowledge and accept the following:**
>
...
VibeRouter is built on top of [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI), an excellent unified proxy server for AI services. Without CLIProxyAPI's robust OAuth handling, token management, and API routing capabilities, this application would not be possible.

**Special thanks to the CLIProxyAPI project and its contributors for creating the foundation that makes VibeRouter work.**
...
**Need Help?**
- Report issues: [GitHub Issues](https://github.com/Geekyshubham/vibeproxy-ultra/issues)
- VibeRouter by Geekyshubham
